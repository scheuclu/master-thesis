...
The Navier stokes equation consist of the \acf{NSME}, the \ac{NSCE} and, if the compressible equations are solved, the \ac{NSEE}. In the literature, the term \ac{NSE} is often synonymously used for the \ac{NSCE}, in this thesis however, with \ac{NSE} we will always refer to the full set of equations ans will explicitly name the momentum, continuity or energy equation otherwise.
...
Bringing the convective form of the compressible \ac{NSE}~\eqref{eq:nsg_compressible} into the conservative form~\eqref{eq:nsg_conservative_form_nsg}, one can derive $\fluxesconv$ and $\fluxesdiff$ as
