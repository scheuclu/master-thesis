...
As explained in section \ref{sec:optimization}, optimization is based on the calculations of gradients, also denoted as sensitivity analysis. When it comes to sensitivity analysis within the context of \ac{CFD}, one has to distinguish between \ac{SA} of the non-coupled fluid problem and the \ac{SA} in the fully coupled aeroelastic case. The latter case is much more involved, since it requires additional terms from the Structure and mesh motion equations. In the following we will therefore begin with the \ac{SA} of a regular fluid problem. A generalization to a coupled FSI problem will follow afterwards.
 \\
...
\end{align}
Therefor, the total derivative must be zero as well:
\begin{align}
...
\PEOSfluidBYmms\DmmsBYabsvarI
\end{align}
In this equation, $\PEOSfluidBYabsvarI$ can be computed analytically or by finite differencing. The derivative of the mesh motion with respect to the abstract variables is often denoted as "shape gradient". It can be divided into two components:
\begin{itemize}
